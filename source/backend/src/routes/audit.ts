...
export async function auditRoutes(app: FastifyInstance) {
  app.get("/api/audit/events", async (req) => {
    ...
    await authz.enforce({ actor, action: "AUDIT_READ", resource: { type: "AUDIT", query: true }, ip: req.ip });
    ...
    return { ...events, events: events.events.map(audit.toApi), chain: await audit.verifyChain(actor.organizationId) };
  });

  app.get("/api/audit/events/:id", async (req) => {
    const actor = requireActor(req);
    const { id } = req.params as { id: string };
    await authz.enforce({ actor, action: "AUDIT_READ", resource: { type: "AUDIT", id }, ip: req.ip });
    const event = await audit.byId(actor.organizationId, id);
    if (!event) throw notFound("Audit event not found.");
    return {
      event: audit.toApi(event),
      trace: (await audit.byTrace(actor.organizationId, event.trace_id)).map(audit.toApi),
      proofs: (await proofService.listProofs(actor.organizationId, { subjectId: event.resource_id ?? undefined })).map(proofService.toApi),
    };
  });
  ...
  app.get("/api/audit/verify-chain", async (req) => {
    const actor = requireActor(req);
    await authz.enforce({ actor, action: "AUDIT_READ", resource: { type: "AUDIT", query: true }, ip: req.ip });
    return await audit.verifyChain(actor.organizationId);
  });

  app.get("/api/audit/export", async (req, reply) => {
    const actor = requireActor(req);
    await authz.enforce({ actor, action: "REPORT_EXPORT", resource: { type: "AUDIT", query: true }, ip: req.ip });
    const q = req.query as any;
    const { events } = await audit.query({ organizationId: actor.organizationId, ...q, limit: 500 });
  ...
  app.get("/api/proofs", async (req) => {
    const actor = requireActor(req);
    await authz.enforce({ actor, action: "PROOF_VERIFY", resource: { type: "PROOF", query: true }, ip: req.ip });
    const q = req.query as any;
    return { proofs: (await proofService.listProofs(actor.organizationId, q)).map(proofService.toApi) };
  });

  app.get("/api/proofs/:id", async (req) => {
    const actor = requireActor(req);
    const { id } = req.params as { id: string };
    await authz.enforce({ actor, action: "PROOF_VERIFY", resource: { type: "PROOF", id }, ip: req.ip });
    const proof = await proofService.getProof(actor.organizationId, id);
  ...
  app.post("/api/proofs/:id/verify", async (req) => {
    ...
    const { id } = req.params as { id: string };
    await authz.enforce({ actor, action: "PROOF_VERIFY", resource: { type: "PROOF", id }, ip: req.ip });
    return await proofService.verifyProof(actor.organizationId, id);
  });
}
