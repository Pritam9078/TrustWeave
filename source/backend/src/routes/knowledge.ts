...
export async function knowledgeRoutes(app: FastifyInstance) {
  app.get("/api/knowledge/documents", async (req) => {
    ...
    authz.enforce({ actor, action: "KNOWLEDGE_READ", resource: { type: "DOCUMENT", query: true }, ip: req.ip });
    return { documents: ragService.listDocuments(actor.organizationId, req.query as any).map(ragService.toApiDocument) };
  });
  ...
  app.post("/api/knowledge/documents", async (req, reply) => {
    ...
    const enforcement = authz.enforce({ actor, action: "KNOWLEDGE_MANAGE", resource: { type: "DOCUMENT", query: true }, ip: req.ip, payload: { title: body.title } });
    const doc = ragService.ingest({ organizationId: actor.organizationId, ...body });
    audit.record({
  ...
  app.delete("/api/knowledge/documents/:id", async (req) => {
    ...
    const enforcement = authz.enforce({ actor, action: "KNOWLEDGE_MANAGE", resource: { type: "DOCUMENT", id }, ip: req.ip });
    ragService.deleteDocument(actor.organizationId, id);
    audit.record({
      organizationId: actor.organizationId, traceId: enforcement.traceId, actorId: actor.identityId,
  ...
  app.post("/api/knowledge/search", async (req) => {
    ...
    return ragService.retrieveForActor(actor, query, Math.min(limit ?? 5, 10));
  });
}
